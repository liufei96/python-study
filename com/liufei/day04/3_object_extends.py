# 03 面向对象之：继承    https://www.cnblogs.com/jin-xin/articles/10274734.html

# 一：什么面向对象的继承？
print('一：什么面向对象的继承？'.center(30, '*'))
...
# 三，单继承。
# 3.1 类名，对象执行父类方法
print('3.1 类名，对象执行父类方法'.center(30, '*'))
...
print(p1)  # <__main__.Person object at 0x0000020D35D1FDF0>
p1.eat()

# 3.2 执行顺序
print('3.2 执行顺序'.center(30, '*'))


class Person(Aniaml):
    def __init__(self, name, sex, age):
        super().__init__(name, sex, age)
        print('子类中的__init__方法')

    def eat(self):
        print('%s 吃饭' % self.name)


p1 = Person('zhangsan', '男', '20')
...
# 3.3同时执行类以及父类方法
print('3.3同时执行类以及父类方法'.center(30, '*'))
'''
方法一：
...
p1 = Person('lisi', '男', 21, '有思想')
print(p1.__dict__) # {'name': 'lisi', 'age': 21, 'sex': '男', 'mind': '有思想'}

# 方法二：利用super，super().func(参数)
print('方法二：利用super，super().func(参数)'.center(40, '-'))
...
for obj in lst:
    obj.func1()  # 111 1 | 111 2 | 222 3


# 四、多继承
print('四、多继承'.center(30, '*'))
class ShenXian: # 神仙
    def fei(self):
        print('神仙都会飞')
...
如图. 肯定是按照123456这样的顺序来送. 那这样的顺序就叫深度优先遍历. ⽽如果是142356呢? 这种被称为⼴度优先遍历. 好了. 深度优先就说这么多. 那么上⾯那个图怎么找的呢? 
MRO是什么呢? 很简单. 记住. 从头开始. 从左往右. ⼀条路跑到头, 然后回头. 继续⼀条路跑到头. 就是经典类的MRO算法. 

类的MRO: Foo-> H -> G -> F -> E -> D -> B -> A -> C. 你猜对了么?
'''

# 4.2新式类的多继承
print('4.2.1 mro序列'.center(30, '*'))
'''
